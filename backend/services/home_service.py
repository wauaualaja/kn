...
async def manager_home(period: Optional[str] = None, entity_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "approvals": await _approval_queue(entity_id, approvals_pending),
...
async def _approval_queue(entity_id: Optional[str], generic_pending: int = 0) -> Dict[str, Any]:
    """Antrean persetujuan dirinci per jenis + tujuan klik-nya (bukan satu angka buram)."""
    scope = _scope(entity_id)
    rows = [
        {"key": "sales_order", "label": "Pesanan penjualan menunggu ACC", "view": "orders",
         "count": await db.sales_orders.count_documents({**scope, "status": "waiting_approval"})},
        {"key": "purchase_order", "label": "Pesanan pembelian menunggu ACC",
         "view": "purchase-approval",
         "count": await db.purchase_orders.count_documents({**scope, "status": "waiting_approval"})},
        {"key": "price", "label": "Permintaan harga khusus", "view": "price-approvals",
         "count": await db.price_approvals.count_documents({**scope, "status": "pending"})},
        {"key": "generic", "label": "Permintaan persetujuan lain", "view": "approval-inbox",
         "count": int(generic_pending or 0)},
    ]
