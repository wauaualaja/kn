...
export default function PurchaseReturns({ currentUser, selectedEntity }) {
  ...
  const matchSearch = (r) => !q || [r.number, r.supplier_name, supName(r.supplier_id), r.po_number, r.debit_note_number]
    .filter(Boolean).some((v) => String(v).toLowerCase().includes(q));
  const filtered = returns.filter((r) => (tab === "all" || r.status === tab) && matchSearch(r));
